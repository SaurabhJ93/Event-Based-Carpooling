...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_enterUser(self):
        response = self.signup(
            "testing@test.com", "password123", "XYZ", "ABC", "1234567890", "Success"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_cityFilter(self, mock_get):
        ...
        response = self.app.get(
            BASE_URL + "/index?filterValue=City&searchValue=Charlotte"
        )
        data = json.loads(response.get_data())
        print("checking response code. is: ", response.status_code)
        mock_get.assert_called_once()
        mock_get.assert_called_with(
            "https://api.seatgeek.com/2/events?venue.city=Charlotte",
            headers={"Content-Type": "application/json"},
            params={"client_id": "MTkwMzc2MzB8MTU3MTc4MjA1Ni41Mw"},
        )
        self.assertEqual(response.status_code, 200)  # Testing if endpoint is hitting
    ...
    def test_dateFilter(self, mock_get):
        ...
        mock_get.return_value = self.sampleResponse
        response = self.app.get(
            BASE_URL + "/index?filterValue=Date&searchValue=2019/11/30"
        )
        data = json.loads(response.get_data())
        print("checking response code. is: ", response.status_code)
        mock_get.assert_called_once()
        mock_get.assert_called_with(
            "https://api.seatgeek.com/2/events?datetime_utc.gte=2019/11/30",
            headers={"Content-Type": "application/json"},
            params={"client_id": "MTkwMzc2MzB8MTU3MTc4MjA1Ni41Mw"},
        )
        self.assertEqual(response.status_code, 200)  # Testing if endpoint is hitting
    ...
    def test_performerFilter(self, mock_get):
        ...
        mock_get.return_value = self.sampleResponse
        response = self.app.get(
            BASE_URL + "/index?filterValue=Performer&searchValue=coachella"
        )
        data = json.loads(response.get_data())
        print("checking response code. is: ", response.status_code)
        mock_get.assert_called_once()
        mock_get.assert_called_with(
            "https://api.seatgeek.com/2/events?performers.slug=coachella",
            headers={"Content-Type": "application/json"},
            params={"client_id": "MTkwMzc2MzB8MTU3MTc4MjA1Ni41Mw"},
        )
        self.assertEqual(response.status_code, 200)  # Testing if endpoint is hitting
    ...
    def test_noFilter(self, mock_get):
        ...
        mock_get.return_value = self.sampleResponse
        response = self.app.get(
            BASE_URL + "/index?filterValue=No%20Filter&searchValue=mets"
        )
        data = json.loads(response.get_data())
        print("checking response code. is: ", response.status_code)
        mock_get.assert_called_once()
        mock_get.assert_called_with(
            "https://api.seatgeek.com/2/events?q=mets",
            headers={"Content-Type": "application/json"},
            params={"client_id": "MTkwMzc2MzB8MTU3MTc4MjA1Ni41Mw"},
        )
        self.assertEqual(response.status_code, 200)  # Testing if endpoint is hitting
