...
from API_Interfaces import API_getEvents_Interface 

class Seat_Geek_Api(API_getEvents_Interface):
    def __init__(self):
        ...
        self.api_end_point = 'events' #this valriable should change depending on Interface called
    ...
    def getEvent(self, event_id):
        response = requests.get(self.api_url_base+self.api_end_point+"/"+event_id, headers=self.header, params=self.params) #get request to api
        if response.raise_for_status() == None: # No exception will be raised when the request is successful
            return response.json() #returning response in json format
        else:
            return None